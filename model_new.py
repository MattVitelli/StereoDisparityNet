...
def conv2d(x, filter_size, stride, num_channels, num_features):
	...
	for x_idx in range(len(x)):
		x_p = tf.nn.conv2d(x[x_idx], W, strides=[1, 1, stride, stride], padding='SAME',
			data_format='NCHW')
		x_p = tf.nn.bias_add(x_p, b, data_format='NCHW')
		x_new.append(x_p)
	return x_new
...
def make_cost_volume_model(x, num_disparities):
	...
	sizes = [int(x[0].shape[0]), int(x[0].shape[1]), 1, int(x[0].shape[2]), int(x[0].shape[3])]
	reshaped_left = tf.reshape(x[0], sizes)
	reshaped_right = tf.reshape(x[1], sizes)
	Lb, Lf, Ld, Ly, Lx = tf.meshgrid(tf.range(0, sizes[0]),
							tf.range(0, sizes[1]),
							tf.range(0, max_d),
							tf.range(0, sizes[3]),
							tf.range(0, sizes[4]),
	...
	tiled_left = tf.tile(reshaped_left, [1, 1, max_d, 1, 1])
	tiled_right = tf.tile(reshaped_right, [1, 1, max_d, 1, 1])

	g_right = tf.gather_nd(tiled_right, tf.stack((Lb, Lf, Ld, Ly, Lx), -1))
